...
+ (dispatch_queue_t)DBOperationQueue
{
	...
	dispatch_once(&onceToken,
                ^{
                  _dbOperationQueue = dispatch_queue_create("SQLiteInstanceManager.DBOperationQueue", DISPATCH_QUEUE_SERIAL);
                });
...
{
	dispatch_retain([self DBOperationQueue]);
	dispatch_sync([self DBOperationQueue], block);
	dispatch_release([self DBOperationQueue]);
}
