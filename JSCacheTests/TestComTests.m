...
@interface TestComTests : AbstractTests

@end

@implementation TestComTests{
     TTDataCacheService *service;
}
...
-(void)before{
    service = [TTDataCacheService sharedInstance];
    [service retain];
...
-(void)testSave{
    ...
    [[SQLiteInstanceManager sharedManager] doInTransactionAsync:^{
        for (int i=0; i<5; i++) {
            TTCacheItem *item = [TTCacheItem new];
...
-(void)testCache{
    ...
    int count = [TTCacheItem countByCriteria:@"where cate_name = '%@'",cateName];
...
-(void)testUpdate{
    ...
    // also updated DB
    NSArray *items = [TTCacheItem findByCriteria:@"where entity_id = '%@'", testEntityId];
    assertThatInt(items.count, equalToInt(1));
    NSDictionary *dataInDB = [(TTCacheItem*)items[0] data];
...
-(void)testDelete{
    ...
    NSInteger countInDB = [TTCacheCategory countByCriteria:@"where name = '%@'",cateName];
    assertThatInt(countInDB, equalToInt(0));
    countInDB = [TTCacheItem countByCriteria:@"where cate_name = '%@'",cateName];
    assertThatInt(countInDB, equalToInt(0));
    ...
    countInDB = [TTCacheItem countByCriteria:@"where entity_id = '%@'",testEntityId1];
    assertThatInt(countInDB, equalToInt(1)); // still in DB
    countInDB = [TTCacheItem countByCriteria:@"where entity_id = '%@'",testEntityId2];
